...
\begin{frame}{Overview}
	\begin{itemize}
		...
		\item Risultati \& Passi successivi
	\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item Quantum metrology \& \textcolor{red}{sensing} \\
...
\begin{itemize}
	...
	\item \textit{Ranging:} misura tempo di volo
\end{itemize}
\vspace{0.2cm}
\textbf{Obiettivo:} Migliorare LiDAR con correlazioni quantistiche tra coppie di fotoni.
...
1 fotone di pompa $\rightarrow$ 2 fotoni, $signal$ e $idler$.
...
Condizioni di \textbf{Phase Matching}: $\omega_p=\omega_s+\omega_i, \ \vec{k}_p = \vec{k}_s + \vec{k}_i$\\
Stato entangled $|TMSV \rangle = \sum_{n} c_n^{\mu_s} |n \rangle_s |n \rangle_i $
...
\begin{frame}{SPDC}
	...
	SPDC processo poco efficiente $\rightarrow $ \textbf{Quasi Phase Matching (QPM)}:\\
...
{\tiny In questo esperimento è stato utilizzato un cristallo $Mg=(5\%):LiNbO_3$ pp su guida d'onda}	
...
\begin{frame}{Quantum Target Detection (QTD)}
	...
	\begin{columns}[T]
		...
		Protocollo: \textbf{Quantum Illumination (QI)}\\

				
		\vspace{0.2cm}
		Target modellizzato da canale rumoroso (Poisson) e con perdite: $\rho_k=\mathcal{E}_{k,\mu_B}(\rho)$\\
		\vspace{0.2cm}
		...
		\begin{center}
			\begin{tikzpicture}
				\node[draw, inner sep=6pt] {
					$
					\mathcal{H}_0:\ \rho_R = \rho_B \otimes \rho_i
					\qquad
					\mathcal{H}_1:\ \rho_R = \rho_k \otimes \rho_i
					$
				};
			\end{tikzpicture}
		\end{center}
		Misura eseguita con stato a multi-copia $\rho^{\otimes L}$.\\
		...
		{\footnotesize
		$\xi_{QCB}(\mathcal{H}_0,\mathcal{H}_1):= \max_{\alpha \in [0,1]} C_\alpha(\rho_0,\rho_1)$ \\ 
		$C_\alpha(\rho_0,\rho_1) := -\log\!\big(\mathrm{Tr}[\rho_0^{\alpha}\rho_1^{1-\alpha}]\big) $
		}
	\end{columns}
...
\begin{frame}{Quantum Target Detection (QTD)}
	\begin{columns}[T]
		\column{0.55\textwidth}
		Con QI si raggiunge vantaggio quantistico di \textbf{6 dB}.\\
		\vspace{0.3cm}
		Problemi:
		\begin{itemize}
			\item 6 dB solo per apparati complessi
			\item storage dell'idler richiede quantum memories
			\item vantaggio solo in situazione ostile:
			\begin{itemize}
				\item  $\mu_s \ll \mu_B$ 
				\item  $k \ll 1$
			\end{itemize}
	...
	\end{columns}
		...
		{\fontsize{5}{2}\selectfont
			S. Lloyd, Science 321, 1463 (2008) \qquad \hspace{2cm}
			Tan et al., Phys. Rev. Lett. 101, 253601 (2008)\\
			J. Shapiro, IEEE A\&E Systems Magazine 35, 8 (2020) \qquad 
...
\begin{frame}{Quantum Target Ranging (QTR)}
	...
	\begin{columns}[T]
		...
		Asse dei tempi discretizzato in $m$ time-bins lunghi $\Delta t$:
		...
		il target si trova in uno degli $m$ time-bin.
		...
		\begin{center}
			\begin{tikzpicture}
				\node[draw, inner sep=6pt] {
					$
					\{ \mathcal{H}_0, \mathcal{H}_1, ... , \mathcal{H}_m \}
					$
				};
			\end{tikzpicture}
		\end{center}
...
\vspace{0.25cm}
	...
	Per lo slot $s$ si ha distribuzione di conteggi:
\[
P_s(\vec n)=\mathrm{Tr}\!\left[\rho_s\,|\vec n\rangle\langle \vec n|\right],
\qquad
|\vec n\rangle=\bigotimes_i |n_i\rangle
\]
\[
\rho_s = \rho_B \otimes \rho_i \ \text{oppure} \ \rho_s = \rho_k \otimes \rho_i
\]
...
Chernoff Information $\xi_Q $ da probabilità classiche trovata con metodi numerici.
...
\begin{frame}{Misure Phase-Insensitive}
	...
	\begin{columns}
		...
		Vale $\xi_{\mathrm{coh}} = 2\mu_B + k\mu_s - 2\sqrt{\mu_B}\sqrt{\mu_B+k\mu_s}$\\
		\vspace{0.2cm}
		...
		\]
		Limite empirico: $\mathcal{Q}_{emp} = 1+\frac{1}{\mu_s}$.
		\begin{tcolorbox}[
			colback=white,
			colframe=black,
			boxrule=0.4pt,
		...
		\column{0.5\textwidth}
		\centering
		\includegraphics[width=1.1\textwidth]{immagini/QvsMean.png}
	\end{columns}
...
\begin{frame}{Analisi dati}
	...
	\begin{tikzpicture}[
		...
		{TimeTagger\\time-bin:\\$\Delta t = 800\,\mathrm{ps}$};
		\node[box, right=18mm of tt] (strings)
		{Stringhe\\di conteggi};
	...
	Si calcola $ C_P = \frac{\sigma_{XY}}{\sigma_X\sigma_Y}$ shiftando le stringhe per trovare picco di correlazione.\\
	Posizione individuata: distanza fra $signal$ e $idler$.
\end{frame}
...
\begin{frame}{Analisi dati}
	...
	Picco di correlazione inizialmente individuato in assenza di rumore.\\
	...
	\begin{columns}
		...
		{\footnotesize $\mu_B \sim 10^{-9}$ (fotoni/bin)}
		...
		{\footnotesize $\mu_B \sim 10^{-6}$ (fotoni/bin)}
		...
		{\footnotesize $\mu_B \sim 10^{-4}$ (fotoni/bin)}
...
Parametri:
\[
\mu_s = (9.83 \pm 7)\cdot 10^{-5} \qquad \kappa = (1.74 \pm 0.05)\cdot 10^{-4} \qquad \eta_{idler} = (9.4 \pm 4)\cdot 10^{-4}
\]
	\begin{columns}
		\column{0.5\textwidth}
...
Risultati preliminari sono comparabili a valori teorici.\\
Differenza (probabilmente) dovuta a instabilità dei parametri nel tempo.
...
\begin{frame}{Risultati}
	...
	\begin{columns}
		...
		\begin{itemize}
			...
			\item $\mu_s \ \text{e} \ \mu_B $ mantenuti bassi per non surriscaldare gli SNSPD
		\end{itemize}
...
\begin{columns}
	...
	\begin{itemize}
		...
		\item Studio teorico a bassi $\mu_s \ \text{e} \ \mu_B$ per trovare zone ad alto $\mathcal{Q}$
...
\begin{frame}{Quantum Target Ranging (QTR)}
	...
	\begin{columns}
		...
		\begin{equation*}
			\xi _{QCB}^{(m)}= \min_{i,j}\xi _{QCB}^{(2)}(\mathcal{H}_i,\mathcal{H}_j)
		\end{equation*}
		Posso scegliere due stati arbitrari $\rho_1 = \rho_k\otimes\rho_B\otimes\cdots \ \text{e} \  \rho_2 = \rho_B\otimes\rho_k\otimes\cdots$, allora
		\begin{equation*}
			\xi_{TR} = 2\,C_{1/2}(\rho_k,\rho_B) \ge \xi_{TD}
		\end{equation*}
		\hspace{3.2cm}  $\downarrow$
		\begin{equation*}
			\boxed{p_{err}^{TR}\le p_{err}^{TD}}
		\end{equation*}
	\end{columns}
	%\vspace{0.4cm}
	%\centering
	%$\xi_{TD} := \max_{\alpha \in [0,1]}C_{\alpha}(\rho_B ,\rho_k) \ \rightarrow \ \xi_{TR} \ \rightarrow \   = \max_{\alpha\in[0,1]} C_\alpha(\rho_1,\rho_2) $
